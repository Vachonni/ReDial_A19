...
TO: A CSV with columns: ConvID - Text - Ratings ("[(UiD, rating)]")
    ...
    ===--->>>  DIFF:  There will always be only one 'real' rating in the list
                      ... teh NEXT one discussed by the RECOMMENDER
                      
                      Not considering further movies in time
                      
                      Not considering movies mentioned by seeker in the future
    ...
    for message in conv_dict['messages']:
      ...
      #                  break
            ...
            if l_movies_rated[count_original_mentions] in movies_found:
                l_ratings_to_come = l_ratings[count_original_mentions:count_original_mentions+1]
            if l_ratings_to_come != []:              
                ...
                # Add data point only if next movie mention is from recommender
                if message['senderWorkerId'] == recommenderID:
